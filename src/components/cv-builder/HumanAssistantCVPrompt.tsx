...
/**
 * Module E — Human Assistant handoff after CV completion.
 *
 * Same honesty standard as the Scholar Assistant's own Human
 * Assistant flow (ES-005G): this captures the student's stated
 * intent and stores it locally, but there is no backend in this
 * project that routes it to an actual person yet. The copy below is
 * written to be accurate about that rather than imply a live
 * connection — see ES-006A Completion Report for the disclosed gap.
 */

const REQUEST_STORAGE_KEY = "kiplan_cv_review_requested";

interface HumanAssistantCVPromptProps {
  open: boolean;
  onClose: () => void;
}

export default function HumanAssistantCVPrompt({ open, onClose }: HumanAssistantCVPromptProps) {
            ...
            {!confirmed ? (
              <>
                <h3 className="font-extrabold text-base text-slate-800 dark:text-white mb-1.5">
                  Would you like our Human Assistant to review your CV before you apply?
                </h3>
